...
function clickDate (index, date) {
  for (let i = 0; i < 35; i++) {
    DAYS.children[i].classList.value.indexOf('active') !== -1 && DAYS.children[i].classList.remove('active')
    // DAYS.children[index].
...
function updateHeaderYear (date) {
  ...
  const oldYear = Number(HYEAR.innerText)
  if (getYear(date) !== Number(HYEAR.innerText)) {
    HYEAR.innerText = getYear(date)
    oldYear < getYear(date) ? slide(HYEAR, 'slideUp') : slide(HYEAR, 'slideDown')
  }
}
...
function updateDays (date) {
  ...
  for (let i = 0; i < len; i++) {
    if (i < firstDay || i > lastDay + firstDay - 1) {
      ...
      innerDay = new Date(new Date(innerDay.setDate(innerDate)).toLocaleDateString()).getTime()
      DAYS.children[i].innerText = innerDate
      DAYS.children[i].classList.value.indexOf('empty') !== -1 && DAYS.children[i].classList.remove('empty')
      DAYS.children[i].classList.value.indexOf('active') !== -1 && DAYS.children[i].classList.remove('active')
      DAYS.children[i].classList.value.indexOf('today') !== -1 && DAYS.children[i].classList.remove('today')
      today === innerDay && DAYS.children[i].classList.add('today')
      DAYS.children[i].onclick = function () { clickDate(i, innerDate) }
    }
  }
}
...
//   if (dom.classList.value.indexOf('slideLeft') !== -1) return
//   dom.classList.add('slideLeft')
//   setTimeout(() => {
//     dom.classList.remove('slideLeft')
//   }, 300)
// }

// function slideRight (dom) {
//   if (dom.classList.value.indexOf('slideRight') !== -1) return
//   dom.classList.add('slideRight')
//   setTimeout(() => {
//     dom.classList.remove('slideRight')
//   }, 300)
// }
// function slideUp (dom) {
//   if (dom.classList.value.indexOf('slideUp') !== -1) return
//   dom.classList.add('slideUp')
//   setTimeout(() => {
//     dom.classList.remove('slideUp')
//   }, 300)
// }
// function slideDown (dom) {
//   if (dom.classList.value.indexOf('slideDown') !== -1) return
